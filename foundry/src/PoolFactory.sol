...
contract LiquidityPool is ReentrancyGuard {
     ...
     */
    function removeLiquidity(uint256 _amount) public nonReentrant onlyOwner {
        ...
        // Update liquidity and LP token balance
        liquidity -= (resultAssetOne * resultAssetTwo);
        lpTokenQuantity[tx.origin] -= (resultAssetOne * resultAssetTwo);
    ...
    function getYield() public onlyOwner {
        ...
        lastYieldFarmedTime[tx.origin] = block.timestamp; // Reentrancy guard
        uint256 yieldSoFar = yieldTaken[tx.origin];
        uint256 userLiquidity = (lpTokenQuantity[tx.origin] * 100) / liquidity;
        uint256 availableYield = ((yield -
            ((yieldSoFar * 100) / userLiquidity)) * userLiquidity) / 100;
        if (availableYield > address(this).balance) {
            revert notEnoughTokens();
        }
        yieldTaken[tx.origin] += availableYield;
        payable(tx.origin).transfer(availableYield);
